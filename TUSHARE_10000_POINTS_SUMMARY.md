# Tushare 10000 积分权限总结

整理日期：2026-06-08
修订日期：2026-09-12（账户权限更正为 10000 积分）
...
10000 积分属于 Tushare 常规 Pro 接口里覆盖更广的档位：

- 可以访问大多数最低积分要求小于等于 10000 的常规接口；具体以官方权限表和接口返回为准。
- 官方频次和数据权限可能调整；采集仍按项目限流器的默认值执行，避免以账户上限替代稳健限流。
- 积分是权限门槛，不是每次查询都会扣积分。
- 分钟数据、实时数据、港美股、新闻、公告、研报、政策库等很多接口属于「独立权限」，不是 10000 积分自动包含。

对当前这个“智能选股系统”项目来说，10000 积分覆盖 A 股日线行情、复权行情、每日指标、财务报表、财务指标、指数行情、行业分类、申万行业日线、资金流、龙虎榜、两融、限售解禁、大宗交易等核心数据，并可按官方表核验此前高于 5000 分门槛的常规接口。

## 2. A 股选股最常用接口

| 数据 | API | 10000 积分账户口径 | 用途 |
|---|---|---:|---|
...
| 申万行业日线 | `sw_daily` | 可用；具体门槛以官方当日权限表为准 | 行业指数行情、估值、市值 |
...
10000 积分也可以覆盖不少跨资产数据：

| 类型 | 常用 API | 10000 积分账户口径 |
|---|---|---:|
| 公募基金 | `fund_basic`, `fund_company`, `fund_nav`, `fund_daily`, `fund_div`, `fund_portfolio`, `fund_adj` | 通常可用，具体门槛以官方表为准 |
| 期货 | `fut_basic`, `fut_daily`, `fut_holding`, `fut_wsr`, `fut_settle` | 可用 |
| 期权 | `opt_basic`, `opt_daily` | 通常可用，具体门槛以官方表为准 |
...
| 政策法规库 | 独立权限 |
| 券商研报库 | 独立权限 |
| 部分特色数据 | 例如筹码分布、盈利预测、券商月度金股等可能需要独立或更高等级权限；以官方表为准 |

## 8. 对本项目的建议

建议优先使用 10000 积分稳定可用的数据，不碰独立权限接口：
